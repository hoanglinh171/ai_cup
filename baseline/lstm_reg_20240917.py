...
#載入訓練資料
dfs = []
data_file = "/home/r26127011/ai_cup/data/avg_data_10min/loc_1"
# loc_file = os.listdir(data_file)
# for loc in loc_file:
  # DataName = os.path.join(data_file, loc)
DataName = os.path.join(data_file, "fold_0/train.csv")
df = pd.read_csv(DataName, encoding='utf-8')
dfs.append(df)
  
SourceData = pd.concat(dfs, ignore_index=True)


# Fill missing value by nearest
features = ['WindSpeed(m/s)','Pressure(hpa)','Temperature(°C)','Humidity(%)','Sunlight(Lux)', 'Power(mW)']
for feature in features:
  SourceData[feature] = SourceData[feature].interpolate(method='nearest')
  SourceData[feature] = SourceData[feature].ffill()
...
# for loc in loc_file:
  # DataName = os.path.join(data_file, loc)
DataName = os.path.join(data_file, "fold_0/test.csv")
df = pd.read_csv(DataName, encoding='utf-8')
dfs.append(df)
  
SourceData = pd.concat(dfs, ignore_index=True)
SourceData['DateTime'] = pd.to_datetime(SourceData['DateTime']).dt.strftime('%Y%m%d%H%M')
LocationStr = SourceData['LocationCode'].astype(int).astype(str).str[-2:]
LocationStr = np.where(SourceData['LocationCode'] < 10, '0' + LocationStr, LocationStr)
SourceData['Serial'] = SourceData['DateTime'].str.cat(LocationStr, sep='')


# Fill missing value by moving average
features = ['WindSpeed(m/s)','Pressure(hpa)','Temperature(°C)','Humidity(%)','Sunlight(Lux)', 'Power(mW)']
for feature in features:
  SourceData[feature] = SourceData[feature].interpolate(method='nearest')
  SourceData[feature] = SourceData[feature].ffill()
...
while(count < len(EXquestion)):
  print('count : ',count)
  LocationCode = EXquestion[count].astype(int)
  strLocationCode = LocationCode.astype(str)[-2:]
  if LocationCode < 10 :
  ...
  for DaysCount in range(len(ReferTitle)):
    if(ReferTitle[DaysCount].astype(int).astype(str)[0][:8] == EXquestion[count].astype(int).astype(str)[0][:8]):
